...
local function onUIActivated(e)
	...
	for _, spell in ipairs(serviceSpells) do
		...
		local label = MenuServiceSpells_Spells:createTextSelect({ text = string.format("%s (%d%s) - %d%s", spell.name, spell.magickaCost, sPoints, spellPrice, sGP) })
		...
		local icon = MenuServiceSpells_Icons:createImage({ path = string.format("icons\\%s", spell.effects[1].object.icon) })
		icon.borderTop = 2
